...
class Channel:
    ...
    def __init__(self, channel_id: str):
        """Экземпляр инициализируется id канала. Дальше все данные будут подтягиваться по API. id канала
        :param channel_id: id канала
        """
        self.__id = channel_id
        self.info = YoutubeParser.take_dict_to_print(channel_id)
    ...
    @staticmethod
    def print_info(dict_to_print):
        """Выводит в консоль информацию о канале."""
        print(json.dumps(dict_to_print, indent=2, ensure_ascii=False, sort_keys=True, separators=(',', ': ')))
